...
//@{-----------------------------------------------------------------
    ...
    inline T normalize()                          { T m = getMagnitude(); x /= m, y /= m; return m; }

    inline bool isZero()                    const { return EQ(x,0) && EQ(y,0); }
    ...
    inline T getMagnitude2()                const { return x*x + y*y; }

    inline T getDistance(const vec2 &rhs)   const { return sqrt(getDistance2(rhs)); }
    inline T getDistance2(const vec2 &rhs)  const { return SQ(rhs.x - x) + SQ(rhs.y - y); }
...
//@{-----------------------------------------------------------------
    static vec3 fromAngles(float theta, float phi) { return vec3(cosf(theta) * cosf(phi), sinf(phi), sinf(theta) * cosf(phi)); }
//@}-----------------------------------------------------------------
